...
data = data.loc[data['Subject_index'] != 39]
...
for i in range(len(features)):
    for clf in clfs:
        sc = 0
        runs = 0
        for idx in [i for i in range(40) if i != 38]:
            print(f"Run for person {idx + 1} for classifier {clf}")
            train, test = get_train_and_test(data, idx + 1)
            score = fit_and_evaluate(clf, train, test, features[i])
            print(f"Got score {score}")
            sc += score
            runs += 1
        sc /= runs
        print("----------------------------------------------")
        print(f"Score for classifier {clf} is {sc} for case of features {i}")
        print("----------------------------------------------")
        print("----------------------------------------------")
